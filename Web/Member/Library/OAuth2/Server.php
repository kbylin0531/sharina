...
*
* @see Controller\ResourceController
* @see Controller\AuthorizeController
* @see Controller\TokenController
*/
    ...
    UserInfoControllerInterface
{
     ...
     * @param TokenType\TokenTypeInterface                     $tokenType           The token type object to use. Valid token types are "bearer" and "mac"
     * @param ScopeInterface                                   $scopeUtil           The scope utility class to use to validate scope
     * @param ClientAssertionType\ClientAssertionTypeInterface $clientAssertionType The method in which to verify the client identity.  Default is HttpBasic
     *
    ...
    public function addStorage($storage, $key = null)
    {
        ...
        if (isset($this->storageMap[$key])) {
            ...
            if ($key === 'client' && !isset($this->storages['client_credentials'])) {
                if ($storage instanceof Storage\ClientCredentialsInterface) {
                    $this->storages['client_credentials'] = $storage;
                }
            } elseif ($key === 'client_credentials' && !isset($this->storages['client'])) {
                if ($storage instanceof Storage\ClientInterface) {
    ...
    protected function createDefaultAuthorizeController()
    {
        if (!isset($this->storages['client'])) {
            throw new \LogicException('You must supply a storage object implementing Storage\ClientInterface to use the authorize server');
        }
