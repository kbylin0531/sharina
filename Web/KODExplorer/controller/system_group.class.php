...
class system_group extends Controller{
	 ...
	 * @param  bool $use_size_add [变更的大小  size_max G为单位   size_use Byte为单位]
	 */
	public static function space_change($the_id,$use_size_add=false){
		$sql = self::load_data();
		$info = $sql->get($the_id);
		if(!is_array($info)){
			show_json('data_not_full',false);
		}
	...
	public function edit() {
		if (!$this->in['group_id']) show_json('data_not_full',false);
	 ...
	 */
	public function del() {
		if (!isset($this->in['group_id'])) show_json('data_not_full',false);
