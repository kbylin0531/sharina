...
/**
 * Class app
 * @property \Sharin\Library\FileCache $sql
 * @package \
 */
class app extends Controller
{
    ...
    {
        parent::__construct();
        $this->sql = \Explorer\Apps::getInstance();
    }
    ...
    public function user_app()
    {
        ...
        $path = \Explorer\Util::dir($this->in['path']);
        if (isset($this->in['action']) && $this->in['action'] == 'add') {
            $path .= '.oexe';
        }

        if (!Util::checkExt($path)) {
        ...
        unset($data['desc']);
        unset($data['group']);
        show_json(file_put_contents($path, json_encode($data)) ? $this->L['success'] : $this->L['error']);
    }
    ...
    {
        $html = \Sharin\Library\Network::getUrlContent($this->in['url']);
