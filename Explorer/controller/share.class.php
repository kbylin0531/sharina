...
*/
use Explorer\Util;
use Sharin\Library\CreatMiniature;


class share extends Controller
{
    /**
     * @var \Sharin\Library\FileCache
     */
    private $sql;
    private $share_info;
    private $share_path;
    ...
    private function _check_share()
    {
        ...
        $this->sql = new \Sharin\Library\FileCache($share_data);
        $list = $this->sql->get();
    ...
    private function _init_info()
    {
        ...
        $share_path = Util::clearDir($this->share_info['path']);
    ...
    {
        return iconv_system(Util::clearDir(rawurldecode($path)));
    }
    ...
    public function folder()
    {
        ...
        if (isset($this->in['path']) && $this->in['path'] != '') {
            $dir = '/' . Util::clearDir($this->in['path']);
        } else {
            $dir = '/';//首次进入系统,不带参数
        }
        $dir = '/' . trim($dir, '/') . '/';
    ...
    {
        $user_config = new \Sharin\Library\FileCache(USER . 'data/config.php');
    ...
    {
        $mime = \Sharin\Library\Mime::getByExt(get_path_ext($this->path));
    ...
    private function zip($zip_path)
    {
        ...
        for ($i = 0; $i < $list_num; $i++) {
            $zip_list[$i]['path'] = Util::clearDir($this->path . $this->_clear($zip_list[$i]['path']));
        }
